...
const ChallanMarkers = ({ challans, zoom }) => {
  ...
  return (
    <>
      {challans.map((c, i) => {
        const color = c.status === 'Active' ? 'blue' : 'red';
        return (
...
const MapBoundsSearch = ({ setChallans, setZoom }) => {
  ...
  const handleSearch = useCallback(() => {
    ...
    axios.get(`${BASE}/reportee/locations?latMin=${latMin}&lngMin=${lngMin}&latMax=${latMax}&lngMax=${lngMax}`)
      .then((res) => {
        const data = res.data;
...
export default function ReporteeMap() {
  ...
  useEffect(() => {
    ...
    if (id) {
      axios.get(`${BASE}/reportee/locations/${id}`)
